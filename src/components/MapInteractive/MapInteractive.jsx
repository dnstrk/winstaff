...
const MapInteractive = ({ setRegion, setOfficesAddr }) => {
    const officesData = officeData["Города"]["Город"];

    const test = officesData.map((off) => off.ЗначенияСвойства);

    // for (var i = 0; i < test.length; i++) {
    //     console.log(test[i][7].Значение);
    //     console.log(test[i][1].Значение);
    // }
    ...
    useEffect(() => {
        ...
        const hover = (region) => {
            ...
            region.onclick = () => {
                ...
                for (var i = 0; i < test.length; i++) {
                    if (region.dataset.title.includes(test[i][7].Значение)) {
                        // console.log(test[i][7].Значение);
                        // console.log(test[i][1].Значение);
                        const addr = test[i][1].Значение;
                        if (test[i][1].Значение.length > 0) {
